#!/usr/bin/env bash
# ==============================================================================
# Google Antigravity (AGY) - Portable Hub (macOS)
# Double-click in macOS Finder or run from terminal: ./agy-portable.command [cmd]
# ==============================================================================

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$SCRIPT_DIR" || exit 1

# Resolve shared root data directory or local data directory
...
check_bin() {
    if [ ! -f "$AGY_BIN" ]; then
        ...
        if [ -n "$SYS_AGY" ] && [ -f "$SYS_AGY" ]; then
            ...
            echo "Run ./setup-bin.sh or place the macOS 'agy' binary into macOS/bin/."
            return 1
        fi
    fi
...
case "$CMD" in
    ...
    menu)
        while true; do
            ...
            echo " [3] Import Login from this Mac"
            echo " [4] Export USB Login to this Mac"
        ...
        ;;
    *)
        # Treat unknown argument as passing to agy CLI
        do_run "$@"
        ;;
esac
